...
class Resource {
    ...
    {
        setExtMap(Resource._xhrTypeMap, extname, xhrType as any);
    }
    ...
    _loadXdr() {
        ...
        const xdr = this.xhr = new (window as any).XDomainRequest(); // eslint-disable-line no-undef
...
module Resource {
     ...
     * @param {number} percentage - The progress of the load in the range [0, 1].
     */
    export type OnProgressSignal = (resource: Resource, percentage: number) => void;
    /**
     * When the resource finishes loading.
     *
     * @memberof Resource
